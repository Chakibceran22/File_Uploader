...
const MyFilesPage: React.FC = () => {
  ...
  const handleCreateFolder = async () => {
    ...
    const isFolderExists = folders.some((folder) => {
      return (
        folder.name === trimedFoldername && folder.parentId === currentFolderId
      );
    });
  ...
  const handleRename = async () => {
    ...
    const folderNameExists = folders.some((folder) => {
      return (
        folder.name == renameValue.trim() && folder.parentId == currentFolderId
      );
    });

    if (folderNameExists) {
      toast.error("this folder name already exists");
      return;
    }
    if (!renameItemId) {
      toast.error("No Folder Id is given");
      return;
    }
    ...
    try {
      const response = await fileService.updateFolderName(
        token,
        updateFolderDTO
      );
      if (!response) {
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    >
                      <IconComponent
                        className={`w-6 h-6 ${getFileColor(
                          file.fileType || ""
                        )}`}
                      />
                    </div>
